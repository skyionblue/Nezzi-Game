...
the level-to-level flow that makes it a sequence instead of a single room.

**Puzzle 2 crossing — decided:** the original design has Scarlet *climb a rope* across the gap,
but vertical climb is an unimplemented stub and reads poorly in the isometric camera. Instead,
Puzzle 2 is reframed as a **Dani-lowered bridge**: Dani pulls a lever, a walkable bridge drops
across the gap, and Scarlet walks over. This preserves the exact beat (separation → Dani enables
Scarlet → relief), is isometric-native, and is buildable now from a `Gate`-style moving platform.
Vertical rope climb is deferred and likely dropped — the `SetClimbingState` / `RopeTrigger` stub
stays in place but unused.
...
- Puzzle 2 requires the switch-and-enable beat: the player must switch to Dani and throw a lever that lowers a bridge across the gap, then Scarlet crosses and both reunite — Puzzle 2 cannot be completed by one character alone.
...
| 2 | Author `World1_Puzzle2` LevelData (The Gap): Scarlet on the near side of a real floor gap she cannot cross (no jump), Dani across the gap with a lever; lever `id` links to a **bridge** that spans the gap when thrown (`oneShot`). Requires a walkable `Gate`-style moving platform — add a `Bridge` variant/prefab (or reuse `Gate` with a walkable top collider) that moves into the gap on plate activation. | L | In play mode, Scarlet cannot cross the gap; switching to Dani and throwing the lever extends/lowers the bridge; Scarlet then walks across and both reunite. One character alone cannot finish; the bridge is solid enough to stand and walk on. | Task 1 (pattern to copy) |
...
## Out of scope
- Rope / vine **vertical climb** mechanic — an explicit stub (`DaniController.SetClimbingState`) that reads poorly in isometric. Puzzle 2 uses a Dani-lowered **bridge** instead; vertical climb is deferred and likely dropped. The `RopeTrigger` / `SetClimbingState` hooks stay in place, unused.
...
- Puzzle 2's "gap" needs a way to make Scarlet unable to cross — with no jump, a literal gap in the floor makes that self-evident; confirm the floor plates leave an actual walkable hole (with a fall-off `Hazard` in the pit).
- **Bridge is the new unknown for Puzzle 2.** It must be a *walkable* moving platform, and standing a `CharacterController` on a moving-collider platform can be finicky. If carrying the character while the bridge animates is troublesome, have the bridge **snap into place instantly** on lever throw rather than animate — the puzzle beat doesn't require motion, only that the crossing appears.
...
- Mechanics: `Assets/Scripts/Characters/ScarletController.cs` (lift), `Assets/Scripts/Characters/DaniController.cs` (stack + climb stub), `Assets/Scripts/Puzzle/{Lever,Gate,PressurePlate,Hazard}.cs`. The Puzzle 2 bridge should reuse/extend `Gate` (moves a platform by an offset on plate activation) as a **walkable** surface.

## Ready to hand to `unity-senior-developer`
- Tasks 1, 6 (LevelData authoring + checkpoint/hazard placement) are well-specified against the existing `World1_Level1.asset` pattern.
- Task 2 (Puzzle 2 + bridge) now includes a small prefab/behavior piece — a walkable `Gate`-style bridge — beyond pure data authoring; hand it off with that note.
