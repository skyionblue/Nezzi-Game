...
namespace OneWayTogether.Core
{
    ...
    public class LevelBuilder : MonoBehaviour
    {
            ...
            foreach (LevelObjectData def in _levelData.objects)
            {
                ...
                switch (def.type)
                {
                    ...
                    case LevelObjectType.StonePressurePlate:
                        SpawnStonePressurePlate(pos, rot, def.id, def.oneShot, def.requiresPhysics);
                        break;
        ...
        private void SpawnStonePressurePlate(Vector3 pos, Quaternion rot, string plateId, bool oneShot = false, bool requiresPhysics = false)
        {
            ...
            if (plate != null)
                plate.Init(plateId, oneShot, requiresPhysics);
        }
